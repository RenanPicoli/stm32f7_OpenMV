...
uint8_t outbytes0[16000] __attribute__ ((section (".dtcmram")));//reduzi o tamanho dos buffers pois reduzi a imagem
uint8_t outbytes1[16000] __attribute__ ((section (".dtcmram")));//reduzi o tamanho dos buffers pois reduzi a imagem
...
#else
//—разу в железо, дл¤ AVR тут стоит #define OUTSYM(VAR) {while(!UCSRA_UDRE);UDR=VAR;}
volatile UINT8 _SYMBOL;
#define OUTSYM(VAR) _SYMBOL=(VAR)
#endif

//»спользовать вместо делени¤ на коэффициент q[i] - умножение на 1/q[i].
#define USE_MUL
...
//    z4 = MULTIPLY(tmp12, FIX_0_306562965) + tmp12 + z5; /* c2+c6 */
    ...
    inp+=IMG_WIDTH; //дл¤ нормальных изображений
  }
  while(--ctr);
}
...
unsigned int jprocess(void)
{
  ...
  return out_jtail(write_pointer);//start poiter нужен дл¤ вычислени¤ размера изображени¤
}
